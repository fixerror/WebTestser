package ua.radchenko.jee.utils;
...
public class ReflectionUtils {
	private static WeakReference<Map<Class<?>, List<Field>>> NOT_STATIC_FIELDS_MAP = new WeakReference<Map<Class<?>, List<Field>>>(
	...
	private static List<Field> getCachedAccessibleFields(Class<?> clazz) {
		...
		if (map == null) {
			synchronized (ReflectionUtils.class) {
				NOT_STATIC_FIELDS_MAP = new WeakReference<Map<Class<?>, List<Field>>>(
						new HashMap<Class<?>, List<Field>>());
	 ...
	 * @param fieldName
	 *            String
	 * @return Field * @throws SecurityException * @throws NoSuchFieldException
	...
	public static Field getAccessibleField(Class<?> clazz, String fieldName)
		...
		throw new NoSuchFieldException(fieldName + " for " + clazz.getName()
				+ " class");
	}
	 ...
	 *            Object
	 */
	public static void copyByFields(List<Field> srcFields,
			List<Field> destFields, Object dest, Object src) {
	...
	public static interface IObjectConverter {
		 ...
		 * @param destClass
		 *            Class<?>
		 * @return Object * @throws IllegalArgumentException * @throws
	...
	private static final class SimpleConverter implements IObjectConverter {
		 ...
		 * @param destClass
		 *            Class<?>
		 * @return Object * @throws IllegalArgumentException * @see
		 *         nedis.htppl.core
		 *         .utils.ReflectionUtils$IObjectConverter#convert(Object,
	 ...
	 *            IObjectConverter
	 */
	public static void copyBeans(Object destBean, Object sourceBean,
			boolean withNullValues, IObjectConverter converter) {
		...
		for (Method setter : methods) {
			...
			if (setterMethodName.startsWith("set")) {
				...
				try {
					...
					try {
						getter = sourceBean.getClass().getMethod(
								getterMethodName);
				...
				if (getter != null) {
					try {
						...
						if (value != null) {
							Object res = converter.convert(value,
	 ...
	 *            Object
	 */
	public static void copyByNotNullFields(List<Field> srcFields,
			List<Field> destFields, Object dest, Object src) {
		for (Field f : srcFields) {
			Field destField = getField(destFields, f.getName());
			if (destField != null) {
